...
def main():
    ...
    send_command(ser, 'name MGMT')
    send_command(ser, 'exit')

    send_command(ser, 'interface vlan 90')
    send_command(ser, 'ip address 192.168.90.5 255.255.255.0')  # <- FIXED IP
    send_command(ser, 'no shutdown')
    send_command(ser, 'exit')

    send_command(ser, 'interface gig1/0/1')  # <- Adjusted for VLAN 90 activation
    send_command(ser, 'switchport mode access')
    send_command(ser, 'switchport access vlan 90')  # <- VLAN 90
    send_command(ser, 'no shutdown')
    send_command(ser, 'exit')

    send_command(ser, 'ip default-gateway 192.168.90.1')  # <- Gateway matching VLAN
